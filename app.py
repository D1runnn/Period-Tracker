...
if len(df) >= 3:
    ...
    gaps = [
        (all_dates[i] - all_dates[i - 1]).days
    ...
    if avg_cycle < 21:
        alerts.append("⚠️ Short cycle length detected.")

    if avg_cycle > 35:
        alerts.append("⚠️ Long cycle length detected.")

    if std_dev > 7:
        alerts.append("⚠️ High cycle variability detected.")

    if days_since > 45:
        alerts.append("⚠️ Possible missed cycle detected.")
    ...
    <div class="status-card" style="border-left-color:{color};">
        <h4 style="margin-top:0;opacity:0.7;">
            CURRENT BIOLOGICAL STATUS
        </h4>

        <h1 style="color:{color};margin-bottom:1rem;">
            {phase}
        </h1>
    ...
    with m1:
        st.metric(
            "Next Period",
            pred_start_avg.strftime("%b %d"),
            f"{(pred_start_avg - today).days} days"
        )

    with m2:
        st.metric(
            "Ovulation",
            ovulation_est.strftime("%b %d"),
            fertility_label
        )
    ...
    with tab1:
        ...
        with col1:
            ...
            follicular_days = max(
                (ovulation_est - last_start).days - mens_days,
                1
            )
        ...
        with col2:
            ...
            st.info(
                f"**Fertile Window Begins**\n\n"
                f"{fertile_start.strftime('%A, %B %d')}"
            )

            st.success(
                f"**Estimated Ovulation**\n\n"
                f"{ovulation_est.strftime('%A, %B %d')}"
            )

            st.error(
                f"**Predicted Period Window**\n\n"
                f"{pred_start_min.strftime('%b %d')} → "
                f"{pred_start_max.strftime('%b %d')}"
            )
        ...
        with col_form:
            ...
            with st.form("log_form", clear_on_submit=True):
                ...
                if submitted:
                    ...
                    # Prevent duplicates
                    elif log_date in df['Date'].dt.date.values:
...
# =========================================================
    ...
    st.info(
        "👋 Welcome! Please log at least "
        "3 previous cycle start dates."
    )
    ...
    with st.form("initial_log"):
        ...
        if submit:
            ...
            if d > today:
                ...
                st.success("Initial date logged.")
